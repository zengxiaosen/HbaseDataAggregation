 ...
 */
public class HBaseReadTableReducer extends TableReducer<Text, Text, ImmutableBytesWritable> {
...
//                    temp.set(6, cf7);
        ...
        for(Map.Entry<Integer, ArrayList<Double>> entry: collectionResult.entrySet()){
            ...
            if(value.get(5)!=null){
                put.add(Bytes.toBytes("flow"), Bytes.toBytes(String.valueOf(key)), Bytes.toBytes(value.get(5).toString()));
            }else{
                put.add(Bytes.toBytes("flow"), Bytes.toBytes(String.valueOf(key)), Bytes.toBytes("0"));
            }

            if(value.get(6)!=null){
                put.add(Bytes.toBytes("latency"), Bytes.toBytes(String.valueOf(key)), Bytes.toBytes(value.get(6).toString()));
            }else{
                put.add(Bytes.toBytes("latency"), Bytes.toBytes(String.valueOf(key)), Bytes.toBytes("0"));
            }

            if(value.get(7)!=null){
                put.add(Bytes.toBytes("request"), Bytes.toBytes(String.valueOf(key)), Bytes.toBytes(value.get(7).toString()));
            }else{
